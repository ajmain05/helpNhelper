...
<div onclick="window.location.href='{{ $url }}'"
    class="cursor-pointer flex flex-col items-start bg-white px-4 py-5 border-1.5 border-gray-400 rounded-[24px] max-w-[484px] @if ($type == 1) h-auto w-full @else h-full @endif">
    @if ($campaign->photo)
        <img src="{{ asset($campaign->photo) }}" alt="Campaign Image" loading="lazy"
            class="object-cover object-center mb-[28px] rounded-[16px] w-full max-w-[452px] h-[370px]">
    @else
        <img src="{{ asset('web-assets/images/fc_image.png') }}" alt="Campaign Image" loading="lazy"
            class="object-cover object-center mb-[28px] rounded-[16px] w-full max-w-[452px] h-[370px]">
    @endif
    <h4 class="text-[28px] font-medium leading-[34px] tracking-[0] mb-[10px] line-clamp-2">{{ $campaign->getTranslation('title') }}</h4>
    <p class="text-[16px] font-medium leading-[28px] tracking-[0] mb-[12px] line-clamp-3">
        {{ $campaign->getTranslation('short_description') }}</p>
    <div class="flex justify-between w-full mb-2">
        <div class="flex w-full items-center gap-1">
            <img src="{{ asset('web-assets/images/love_icon.svg') }}" loading="lazy" alt="Love icon">
            <p class="text-[16px] font-medium leading-[19px] tracking-[0%]">{{ $campaign->total_donors }} {{ __('Supporters') }}</p>
        </div>
        <p class="text-[18px] font-medium leading-[28px] tracking-[0]">{{ $percentage }}%</p>
    </div>
    <div class="w-full flex flex-col mb-5">
        ...
        <span role="progressbar" aria-labelledby="ProgressLabel" aria-valuenow="75"
            class="block rounded-full bg-[#1799ea4d]">
            <span class="block h-3 rounded-full bg-[rgb(23_153_234)]" style="width: {{ $percentage }}%"></span>
        </span>
    </div>
    <div class="flex justify-between w-full items-start flex-col gap-3 xl:flex-row xl:gap-0">
        <div class="flex flex-col w-full">
            <h5 class="text-[24px] font-semibold leading-[37px] tracking-[0] mb-1">BDT
                {{ number_format($campaign->total_donation) }}
            </h5>
            <p class="text-[14px] font-medium leading-[17px] tracking-[0%] text-[#6F7279]">{{ __('raised of') }}
                BDT {{ number_format($campaign->amount, 2) }}
                {{ __('goal') }}</p>
        </div>
        
        <div class="flex items-center gap-2">
            <!-- Share Button -->
            <button onclick="event.stopPropagation(); toggleShareModal('{{ $campaign->id }}', '{{ addslashes($campaign->getTranslation('title')) }}', '{{ route('campaign', $campaign->id) }}')" 
                class="flex justify-center items-center bg-[#1799ea] hover:bg-[#1589d1] px-6 py-2 rounded-[100px] transition-colors h-[52px]">
                <span class="text-[16px] font-semibold leading-7 tracking-normal text-white">{{ __('Share') }}</span>
            </button>

            <a href="{{ route('donation', $campaign->id) }}"
                class="flex justify-between gap-2 items-center bg-[#27E36A] pl-[24px] pr-2 py-2 rounded-[100px] w-full min-w-[140px]">
...
<!-- Social Share Modal -->
<div id="shareModal-{{ $campaign->id }}" onclick="event.stopPropagation(); closeShareModal('{{ $campaign->id }}')" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm hidden opacity-0 transition-opacity duration-300">
    <div onclick="event.stopPropagation()" class="bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-2xl w-[90%] max-w-md transform scale-95 transition-transform duration-300 border border-white/20">
        <div class="flex justify-between items-center mb-6">
            ...
            <button onclick="closeShareModal('{{ $campaign->id }}')" class="p-2 hover:bg-gray-100 rounded-full transition-colors">
        ...
        <div class="relative">
            ...
            <div class="flex items-center gap-2 bg-gray-50 rounded-lg p-2 border border-gray-200">
                ...
                <button onclick="copyToClipboard('{{ $url }}')" class="p-2 text-gray-500 hover:text-green-600 transition-colors" title="{{ __('Copy Link') }}">
...
<script>
    function toggleShareModal(id, title = '', url = '') {
